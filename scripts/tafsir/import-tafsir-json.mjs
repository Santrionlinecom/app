...
const usage = `SantriOnline Tafsir Indonesia JSON importer

Usage:
  node scripts/tafsir/import-tafsir-json.mjs --input data/tafsir/sample-tafsir.json [options]
...
const validateEntry = (input, index) => {
	...
	const ayahNumber = toInteger(input.ayah_number);
	const content = cleanText(input.content, 40_000);
	...
	if (!ayahNumber || ayahNumber < 1) errors.push('ayah_number wajib angka positif');
	if (!content) errors.push('content wajib diisi');

	if (errors.length || !surahNumber || !ayahNumber) {
		return {
			ok: false,
...
const entryToInsertSql = (entry) => {
	...
	return `INSERT INTO quran_tafsir_entries (
  source_key,
  surah_number,
  ayah_number,
  title,
...
SELECT
  ...
  CURRENT_TIMESTAMP
WHERE EXISTS (
  SELECT 1 FROM quran_tafsir_sources WHERE source_key = ${sourceKey}
)
ON CONFLICT(source_key, surah_number, ayah_number) DO UPDATE SET
...
const main = async () => {
	...
	const tempDir = await mkdtemp(path.join(os.tmpdir(), 'santrionline-tafsir-'));
	try {
		for (const [index, sql] of sqlBatches.entries()) {
			const batchFile = path.join(tempDir, `tafsir-batch-${String(index + 1).padStart(3, '0')}.sql`);
