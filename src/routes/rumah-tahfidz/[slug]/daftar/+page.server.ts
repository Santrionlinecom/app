...
export const load: PageServerLoad = async ({ params, locals, url }) => {
	...
	const lockedRoleValue = getMemberReferralRole('rumah-tahfidz', url);

	return {
		org,
		lockedRole: lockedRoleValue ? { value: lockedRoleValue, label: roleLabel(lockedRoleValue) } : null,
		roles: allowedRolesByType['rumah-tahfidz'].filter((role) => role !== 'ustadzah').map((role) => ({
			value: role,
...
export const actions: Actions = {
	default: async ({ request, locals, cookies, params, url, platform }) => {
		const blockedAction = getInstitutionActionBlock('rumah-tahfidz');
		if (blockedAction) {
		...
		if (!org) {
			return fail(404, { error: 'Rumah Tahfidz tidak ditemukan' });
		}
		if (org.status !== 'active') {
			return fail(400, { error: 'Rumah Tahfidz belum aktif' });
		}
		...
		if (!allowedRolesByType['rumah-tahfidz'].includes(roleValue as any)) {
		...
		await logActivity(db, {
			userId,
			action: 'REGISTER',
			metadata: { orgId: org.id, orgName: org.name, orgType: 'rumah-tahfidz', source: 'rumah-tahfidz/member' }
		});
		...
		await logActivity(db, {
			userId,
			action: 'LOGIN',
			metadata: { method: 'password', source: 'rumah-tahfidz/member' }
		});
