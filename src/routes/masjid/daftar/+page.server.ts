	...
	default: async ({ request, locals, cookies, platform }) => {
		...
		if (isLoggedIn && locals.user) {
			...
			await logActivity(db, {
				userId,
				action: 'REGISTER',
				metadata: { orgId, orgName: orgName.trim(), orgType: 'masjid', source: 'masjid/daftar' }
			});
			logSystemActivity(db, 'REGISTER', {
				userId,
				userEmail: (adminEmail as string).trim(),
				ipAddress: getRequestIp(request),
				metadata: { orgId, orgName: orgName.trim(), orgType: 'masjid', role: 'admin', source: 'masjid/daftar' },
			...
			await logActivity(db, {
				userId,
				action: 'LOGIN',
				metadata: { method: 'password', source: 'masjid/daftar' }
			});
		}
