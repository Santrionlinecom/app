		...
		<section>
			<article class="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
				...
				<div class="mt-4 rounded-[1.5rem] border border-emerald-100 bg-emerald-50 px-5 py-5">
					<p class="text-xs font-semibold uppercase tracking-[0.28em] text-emerald-600">Disclaimer</p>
					...
					{#each chapterMap as chapter, index}
						...
						<article class="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
							...
							{#if chapterHref}
								<a
									href={chapterHref}
									class="mt-4 inline-flex rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600"
								>
									Buka Bab
								</a>
							{/if}
				...
				{#each curatedItem.modules as module, index}
					...
					>
						...
						<div class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
							...
							<span class="rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition group-hover:bg-emerald-600">
								Buka Bab
							</span>
						</div>
					</a>
		...
		<section class="grid gap-6 xl:grid-cols-[1.12fr_0.88fr]">
			<article id="reader-sumber" class="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
				<div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
					...
					{#if downloadUrl}
						<a href={downloadUrl} target="_blank" rel="noreferrer" class="text-sm font-semibold text-emerald-600 hover:text-emerald-600">
							Buka di tab baru
						</a>
					{/if}
				</div>
