...
<section class="space-y-6">
	<header class="rounded-3xl border bg-gradient-to-r from-emerald-600 to-teal-600 p-6 text-white shadow-xl">
		...
		<p class="text-sm text-white/90">Approve pondok, masjid, dan musholla yang mendaftar.</p>
	</header>
					...
					<tbody>
						{#each pending as org}
							<tr>
								...
								<td class="text-slate-600">{org.type}</td>
			...
			<div class="grid gap-3 md:grid-cols-2">
				{#each active as org}
					<div class="rounded-xl border p-4">
						...
						<div class="text-sm text-slate-600">{org.type} • {org.slug}</div>
					</div>
				{/each}
			</div>
		{/if}
