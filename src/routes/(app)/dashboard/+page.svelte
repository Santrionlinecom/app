<script lang="ts">
	...
	// Pemuatan malas panel per peran.
	// Impor hanya dijalankan saat panelnya memang akan ditampilkan, sehingga
	// pengguna tidak mengunduh markup peran lain yang tak pernah dirender.
	let tpqPanelPromise: Promise<any> | null = null;
	let communityPanelPromise: Promise<any> | null = null;

	$: if (orgType === 'tpq' && tpqDashboard && !tpqPanelPromise) {
		tpqPanelPromise = import('$lib/components/dashboard/panels/TpqPanel.svelte');
	}
	$: if (isCommunityOrg && !communityPanelPromise) {
		communityPanelPromise = import('$lib/components/dashboard/panels/CommunityPanel.svelte');
	}
...
<div class="dashboard-command-shell mx-auto w-full max-w-[90rem] space-y-6 text-so-ink">
	...
	{#if orgType === 'tpq' && tpqDashboard}
		{#await tpqPanelPromise then module}
			{#if module}
				<svelte:component
					this={module.default}
					{tpqCards}
					...
					{formatDate}
					{getSurahName}
					{getSetoranStatusLabel}
					{getSetoranStatusClass}
				/>
			{/if}
		{/await}
	{/if}
	...
	{#if isCommunityOrg}
		{#await communityPanelPromise then module}
			{#if module}
				<svelte:component
					this={module.default}
					{finance}
