		...
		<div class="rounded-2xl border bg-white p-4 shadow-sm">
			...
			<form class="mt-4 space-y-4" method="POST" action="?/generate">
				...
				<div class="rounded-xl border border-emerald-100 bg-emerald-50 p-3">
					<p class="text-xs font-semibold text-emerald-600">Features</p>
					<div class="mt-2 flex flex-wrap gap-2">
						{#each selectedProduct?.features ?? [] as feature}
							<span class="badge badge-outline border-emerald-300 text-emerald-600">{feature}</span>
						{/each}
					</div>
				</div>
			...
			{#if form?.success && form.licenseKey}
				<div class="mt-4 space-y-4">
					<div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 sm:p-4">
						<p class="text-xs font-medium uppercase tracking-wide text-emerald-600">License Key</p>
