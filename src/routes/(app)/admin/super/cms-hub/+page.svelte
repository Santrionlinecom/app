...
<div class="cms-hub space-y-6 text-slate-900">
	<nav class="flex gap-2 overflow-x-auto pb-1" aria-label="Navigasi section CMS Hub">
		{#each hubNavItems as item}
			{@const Icon = item.icon}
			<a href={item.href} class="cms-nav-link shrink-0">
	...
	<div class="rounded-2xl border border-emerald-200 bg-emerald-50/90 px-4 py-3 sm:px-5">
		<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
			...
			<a
				href="#sales-chart"
				class="inline-flex h-10 shrink-0 items-center justify-center rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white"
			>
				Cek penjualan
			</a>
		</div>
