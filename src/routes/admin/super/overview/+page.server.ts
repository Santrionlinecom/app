...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const notificationSince = Date.now() - 24 * 60 * 60 * 1000;

	const recentRegisterNotifications = await safeLogQuery(async () => {
		const { results } = await db
			.prepare(
	...
	}, [] as Array<{ id: string; kind: string; severity: string; title: string; body: string; href: string; createdAt: number | null }>);

	const pendingTopupNotifications = await safeLogQuery(async () => {
		const { results } = await db
			.prepare(
	...
	const chatNotifications = await safeLogQuery(async () => {
		const { results } = await db
			.prepare(
	...
	const pendingInstitutionNotifications = orgRows
	...
	const noAdminNotifications = orgRows
		.filter((org) => !org.adminCount)
		.slice(0, 5)
		.map((org) => ({
			id: `org-admin:${org.id}`,
			kind: 'institution',
	...
	const pendingSaleNotifications = digitalCommerce.pendingSales.slice(0, 6).map((sale) => ({
		id: `digital-sale:${sale.id}`,
		kind: 'order',
		severity: 'urgent',
		title: 'Order digital menunggu verifikasi',
		body: `${sale.buyerName || sale.buyerContact || 'Pembeli'} - ${sale.productTitle || sale.referenceCode}`,
		href: '#digital-sales',
	...
	const notifications = [
		...
		.sort((left, right) => {
			const rank: Record<string, number> = { urgent: 3, warning: 2, info: 1 };
