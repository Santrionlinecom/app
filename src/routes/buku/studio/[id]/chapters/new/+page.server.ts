...
export const actions: Actions = {
	default: async ({ request, locals, platform, params }) => {
		...
		try {
			await createBukuChapter(db, book.id, parsed.values);
		} catch (err: any) {
			...
			return fail(500, { error: 'Gagal membuat bab.', values: parsed.values });
		}

		throw redirect(303, `/buku/studio/${book.id}/edit#chapters`);
	}
};
