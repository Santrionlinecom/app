...
<section class="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm sm:p-5">
	<div class="mb-3 flex flex-wrap items-center justify-between gap-2">
		<div>
			<p class="text-sm font-bold text-slate-900">Bagikan ke ummah lembaga</p>
			<p class="text-xs text-slate-500">Ilmu · adab · amal (opsional) · skill — tanpa aib orang lain</p>
		</div>
	...
	<div class="flex items-start gap-3">
		...
		<div class="min-w-0 flex-1">
			<textarea
				...
				placeholder="Apa manfaat yang ingin dibagikan hari ini? (ilmu, adab, habit, atau karya)"
			></textarea>
			<div class="mt-3">
				<MediaUpload
					{mediaUrl}
			...
			<div class="mt-4 flex flex-wrap items-center justify-between gap-3">
				...
				<button
					type="button"
					class="inline-flex items-center justify-center rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300"
					disabled={!canPost}
					on:click={submitPost}
				>
					{isPosting ? 'Memposting...' : isUploadingMedia ? 'Mengupload foto...' : 'Bagikan'}
				</button>
			</div>
			{#if errorMessage}
