		...
		<section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
			...
			<ul class="mt-4 space-y-3">
				{#each visibleRelatedPosts as post}
					<li>
						<a href={`/blog/${post.slug}`} rel="related" class="block font-semibold leading-6 text-slate-700 hover:text-emerald-700">
							{post.title}
						</a>
					</li>
				{/each}
	...
	<section class="rounded-lg border border-slate-200 bg-slate-50 p-5">
		...
		<div class="mt-4 grid gap-2">
			{#each popularPages as page}
				<a href={page.url} class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-emerald-200 hover:text-emerald-700">
					<span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-emerald-50 text-[11px] font-bold text-emerald-700">
						{page.icon}
					</span>
